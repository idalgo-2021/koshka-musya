...
export function useAssignments() {
  ...
  const fetchAssignments = React.useCallback(async (page = 1, limit = 20, retry = false, listingType?: number, city?: string) => {
    if (!retry) {
      setLoading(true);
      setError(null);
    }
    ...
    try {
      ...
      const offeredResponse = await AssignmentsApi.getAvailableAssignments(page, limit, listingType, city);
